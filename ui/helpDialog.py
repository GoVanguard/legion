...
class Validate(QtCore.QObject):                                         # used to validate user input on focusOut - more specifically only called to validate tool name in host/port/terminal commands tabs
    def eventFilter(self, widget, event):
        if event.type() == QtCore.QEvent.FocusOut:                      # this horrible line is to avoid making the 'AddHelpDialog' class visible from here
...
class HelpTabBarWidget(QtWidgets.QTabBar):
...
class AddHelpDialog(QtWidgets.QDialog):                                 # dialog shown when the user selects help menu
    def __init__(self, parent=None):
        ...
        self.previousTab = self.helpTabWidget.tabText(self.helpTabWidget.currentIndex())
    ...
    def setupConnections(self):
        ...
        self.helpTabWidget.currentChanged.connect(self.switchTabClick)
        self.ToolHelpTab.currentChanged.connect(self.switchToolTabClick)

    ##################### ACTION FUNCTIONS (apply / cancel related) #####################

    def setHelp(self, help):                                    # called by the controller once the config file has been read at start time and also when the cancel button is pressed to forget any changes.
        self.help = help
        self.resetGui()                                                 # clear any changes the user may have made and canceled.
        self.populateHelp()                                         # populate the GUI with the new help
    ...
    def applyHelp(self):                                            # called when apply button is pressed
        if self.validateCurrentTab(self.helpTabWidget.tabText(self.helpTabWidget.currentIndex())):
            self.updateHelp()
            return True
        return False
    
    def updateHelp(self):                                           # updates the local help object (must be called when applying help and only after validation succeeded)
                                                                        # LEO: reorganised stuff in a more logical way but no changes were made yet :)
        # update GENERAL tab help
        self.help.general_default_terminal = str(self.terminalComboBox.currentText())
        self.help.general_max_fast_processes = str(self.fastProcessesComboBox.currentText())
        self.help.general_screenshooter_timeout = str(self.screenshotTextinput.text())
        self.help.general_web_services = str(self.webServicesTextinput.text())      

        if self.checkStoreClearPW.isChecked():
            self.help.brute_store_cleartext_passwords_on_exit = 'True'
        else:
            self.help.brute_store_cleartext_passwords_on_exit = 'False'
        
        if self.checkBlackBG.isChecked():
            self.help.general_tool_output_black_background = 'True'
        else:
            self.help.general_tool_output_black_background = 'False'

        # update BRUTE tab help
        self.help.brute_username_wordlist_path = str(self.userlistPath.text())
        self.help.brute_password_wordlist_path = str(self.passwordlistPath.text())
        self.help.brute_default_username = str(self.defaultUserText.text())
        self.help.brute_default_password = str(self.defaultPassText.text())

        # update TOOLS tab help
        self.help.tools_nmap_stage1_ports = str(self.stage1Input.text())
        self.help.tools_nmap_stage2_ports = str(self.stage2Input.text())
        self.help.tools_nmap_stage3_ports = str(self.stage3Input.text())
        self.help.tools_nmap_stage4_ports = str(self.stage4Input.text())
        self.help.tools_nmap_stage5_ports = str(self.stage5Input.text())

        # update AUTOMATED ATTACKS tab help
        if self.enableAutoAttacks.isChecked():
            self.help.general_enable_scheduler = 'True'
        else:
            self.help.general_enable_scheduler = 'False'
            
        # TODO: seems like all the other help should be updated here as well instead of updating them in the validation function.
    ...
    def populateHelp(self):                                         # called by setHelp at start up or when showing the help dialog after a cancel action. it populates the GUI with the controller's help object.
    ...
    def populateGeneralTab(self):       
        ...
        self.fastProcessesComboBox.setCurrentIndex(int(self.help.general_max_fast_processes) - 1)
        self.screenshotTextinput.setText(str(self.help.general_screenshooter_timeout))
        self.webServicesTextinput.setText(str(self.help.general_web_services))
                
        if self.help.general_tool_output_black_background == 'True' and self.checkBlackBG.isChecked() == False:
            self.checkBlackBG.toggle()
        elif self.help.general_tool_output_black_background == 'False' and self.checkBlackBG.isChecked() == True:
            self.checkBlackBG.toggle()
        
        if self.help.brute_store_cleartext_passwords_on_exit == 'True' and self.checkStoreClearPW.isChecked() == False:
            self.checkStoreClearPW.toggle()
        elif self.help.brute_store_cleartext_passwords_on_exit == 'False' and self.checkStoreClearPW.isChecked() == True:
            self.checkStoreClearPW.toggle()     

    def populateBruteTab(self):
        self.userlistPath.setText(self.help.brute_username_wordlist_path)
        self.passwordlistPath.setText(self.help.brute_password_wordlist_path)           
        self.defaultUserText.setText(self.help.brute_default_username)
        self.defaultPassText.setText(self.help.brute_default_password)  
            
    def populateToolsTab(self): 
        # POPULATE TOOL PATHS TAB
        self.nmapPathInput.setText(self.help.tools_path_nmap)
        self.hydraPathInput.setText(self.help.tools_path_hydra)
        self.cutycaptPathInput.setText(self.help.tools_path_cutycapt)
        self.textEditorPathInput.setText(self.help.tools_path_texteditor)
 
        # POPULATE STAGED NMAP TAB
        self.stage1Input.setText(self.help.tools_nmap_stage1_ports)
        self.stage2Input.setText(self.help.tools_nmap_stage2_ports)
        self.stage3Input.setText(self.help.tools_nmap_stage3_ports)
        self.stage4Input.setText(self.help.tools_nmap_stage4_ports)
        self.stage5Input.setText(self.help.tools_nmap_stage5_ports)

        # POPULATE TOOLS TABS (HOST/PORT/TERMINAL)
        self.toolForHostsTableWidget.setRowCount(len(self.help.hostActions))
        for row in range(len(self.help.hostActions)):
            # add a row to the table
            self.toolForHostsTableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem())
            # add the label for the port actions
            self.toolForHostsTableWidget.item(row, 0).setText(self.help.hostActions[row][1])

        self.toolForServiceTableWidget.setRowCount(len(self.help.portActions))
        for row in range(len(self.help.portActions)):
            self.toolForServiceTableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem())
            self.toolForServiceTableWidget.item(row, 0).setText(self.help.portActions[row][1])

        self.servicesAllTableWidget.setRowCount(len(self.help.portActions))
        for row in range(len(self.help.portActions)):
            self.servicesAllTableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem())
            self.servicesAllTableWidget.item(row, 0).setText(self.help.portActions[row][3])

        self.toolForTerminalTableWidget.setRowCount(len(self.help.portTerminalActions))
        for row in range(len(self.help.portTerminalActions)):
            # add a row to the table
            self.toolForTerminalTableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem())
            # add the label fro the port actions
            self.toolForTerminalTableWidget.item(row, 0).setText(self.help.portTerminalActions[row][1])
        self.terminalServicesAllTable.setRowCount(len(self.help.portTerminalActions))
        for row in range(len(self.help.portTerminalActions)):
            self.terminalServicesAllTable.setItem(row, 0, QtWidgets.QTableWidgetItem())
            self.terminalServicesAllTable.item(row, 0).setText(self.help.portTerminalActions[row][3])
                
    def populateAutomatedAttacksTab(self):                              # TODO: this one is still to big and ugly. needs work.
        self.typeDic = {}
        for i in range(len(self.help.portActions)):
            # the dictionary contains the name, the text input and the layout for each tool
            self.typeDic.update({self.help.portActions[i][1]:[QtWidgets.QLabel(),QtWidgets.QLineEdit(),QtWidgets.QCheckBox(),QtWidgets.QHBoxLayout()]})

        for keyNum in range(len(self.help.portActions)):
            
            # populate the automated attacks tools tab with every tool that is not a default creds check
            if self.help.portActions[keyNum][1] not in self.defaultServicesList: 

                self.typeDic[self.help.portActions[keyNum][1]][0].setText(self.help.portActions[keyNum][1])
                self.typeDic[self.help.portActions[keyNum][1]][0].setFixedWidth(150)

                #if self.help.portActions[keyNum][1] in self.help.automatedAttacks.keys():
                foundToolInAA = False
                for t in self.help.automatedAttacks:
                    if self.help.portActions[keyNum][1] == t[0]:    
                        #self.typeDic[self.help.portActions[keyNum][1]][1].setText(self.help.automatedAttacks[self.help.portActions[keyNum][1]])
                        self.typeDic[self.help.portActions[keyNum][1]][1].setText(t[1])
                        self.typeDic[self.help.portActions[keyNum][1]][2].toggle()
                        foundToolInAA = True
                        break
                        
                if not foundToolInAA:
                    self.typeDic[self.help.portActions[keyNum][1]][1].setText(self.help.portActions[keyNum][3])

                self.typeDic[self.help.portActions[keyNum][1]][1].setFixedWidth(300)
                self.typeDic[self.help.portActions[keyNum][1]][2].setObjectName(str(self.typeDic[self.help.portActions[keyNum][1]][2]))
                self.typeDic[self.help.portActions[keyNum][1]][3].addWidget(self.typeDic[self.help.portActions[keyNum][1]][0])
                self.typeDic[self.help.portActions[keyNum][1]][3].addWidget(self.typeDic[self.help.portActions[keyNum][1]][1])
                self.typeDic[self.help.portActions[keyNum][1]][3].addItem(self.enabledSpacer)
                self.typeDic[self.help.portActions[keyNum][1]][3].addWidget(self.typeDic[self.help.portActions[keyNum][1]][2])
                self.scrollVerLayout.addLayout(self.typeDic[self.help.portActions[keyNum][1]][3])
                ...
                if self.help.portActions[keyNum][1] == "mysql-default":
                    self.typeDic[self.help.portActions[keyNum][1]][0].setText('mysql')
                elif self.help.portActions[keyNum][1] == "mssql-default":
                    self.typeDic[self.help.portActions[keyNum][1]][0].setText('mssql')
                elif self.help.portActions[keyNum][1] == "ftp-default":
                    self.typeDic[self.help.portActions[keyNum][1]][0].setText('ftp')
                elif self.help.portActions[keyNum][1] == "postgres-default":
                    self.typeDic[self.help.portActions[keyNum][1]][0].setText('postgres')
                elif self.help.portActions[keyNum][1] == "oracle-default":
                    self.typeDic[self.help.portActions[keyNum][1]][0].setText('oracle')
                ...
                self.defaultBoxVerlayout.addLayout(self.typeDic[self.help.portActions[keyNum][1]][3])
    ...
    def switchTabClick(self):                                           # LEO: this function had duplicate code with validateCurrentTab(). so now we call that one.
        if self.helpTabWidget.tabText(self.helpTabWidget.currentIndex()) == 'Tools':
            self.previousToolTab = self.ToolHelpTab.tabText(self.ToolHelpTab.currentIndex())
    ...
    def switchToolTabClick(self):                                       # TODO: check for duplicate code.
        if self.ToolHelpTab.tabText(self.ToolHelpTab.currentIndex()) == 'Host Commands':
            self.toolForHostsTableWidget.selectRow(0)
            self.updateToolForHostInformation(False)
            
        elif self.ToolHelpTab.tabText(self.ToolHelpTab.currentIndex()) == 'Port Commands':
            self.toolForServiceTableWidget.selectRow(0)
            self.updateToolForServiceInformation(False)
            
        elif self.ToolHelpTab.tabText(self.ToolHelpTab.currentIndex()) == 'Terminal Commands':
        ...
        if self.previousToolTab == 'Tool Paths':
            if not self.toolPathsValidate():
                self.ToolHelpTab.setCurrentIndex(0)
            ...
            if not self.validateCommandTabs(self.hostActionNameText, self.hostLabelText, self.hostCommandText):
                self.ToolHelpTab.setCurrentIndex(1)
            else:
            ...
            if not self.validateCommandTabs(self.portActionNameText, self.portLabelText, self.portCommandText):
                self.ToolHelpTab.setCurrentIndex(2)
            else:
...
#               self.updateTerminalActions()                            # LEO: commented out because it didn't look right, please check!
    ...
    def resetTabIndexes(self):                                          # called when the help dialog is opened so that we always show the same tabs.
    ...
    def validateCurrentTab(self, tab):                                  # LEO: your updateHelp() was split in 2. validateCurrentTab() and updateHelp() since they have different functionality. also, we now have a 'tab' parameter so that we can reuse the code in switchTabClick and avoid duplicate code. the tab parameter will either be the current or the previous tab depending where we call this from.
        ...
        if tab == 'General':
            ...
            self.ToolHelpTab.setCurrentIndex(0)
            currentToolsTab = self.ToolHelpTab.tabText(self.ToolHelpTab.currentIndex())
...
#   def commandTabsValidate(self):                                      # LEO: renamed and refactored
    ...
    def selectGroup(self):      
        ...
        if self.previousToolTab == 'Terminal Commands' or tabSelected == 3:
            ...
            actions = self.help.portTerminalActions
...
#   def validateInput(self):                                            # LEO: renamed
    ...
    def addToolForHost(self):
        ...
        if self.validateCommandTabs(self.hostActionNameText, self.hostLabelText, self.hostCommandText):
            ...
            self.help.hostActions.append(['', 'New_Action_'+str(self.hostActionsNumber), ''])
    ...
    def removeToolForHost(self):
        ...
        for tool in self.help.hostActions:
            if tool[1] == str(self.hostActionNameText.text()):
                self.help.hostActions.remove(tool)
                break
    ...
    def updateHostActions(self):
        self.help.hostActions[self.hostTableRow][0] = str(self.hostLabelText.text())
        self.help.hostActions[self.hostTableRow][2] = str(self.hostCommandText.text())
...
#           self.hostCommandText.setStyleSheet("border: 1px solid grey;")
    ...
    def addToolForService(self):
        ...
        if self.validateCommandTabs(self.portActionNameText, self.portLabelText, self.portCommandText):
            ...
            self.help.portActions.append(['', 'New_Action_'+str(self.portActionsNumber), ''])
            self.portActionsNumber +=1
    ...
    def removeToolForService(self):
        ...
        for tool in self.help.portActions:
            if tool[1] == str(self.portActionNameText.text()):
                self.help.portActions.remove(tool)
                break
    ...
    def updatePortActions(self):
        self.help.portActions[self.portTableRow][0] = str(self.portLabelText.text())
        self.help.portActions[self.portTableRow][2] = str(self.portCommandText.text())
...
#           self.portCommandText.setStyleSheet("border: 1px solid grey;")
    ...
    def addToolForTerminal(self):
        ...
        if self.validateCommandTabs(self.terminalActionNameText, self.terminalLabelText, self.terminalCommandText):
            ...
            self.help.portTerminalActions.append(['', 'New_Action_'+str(self.terminalActionsNumber), ''])
            self.terminalActionsNumber +=1
    ...
    def removeToolForTerminal(self):
        ...
        for tool in self.help.portTerminalActions:
            if tool[1] == str(self.terminalActionNameText.text()):
                self.help.portTerminalActions.remove(tool)
                break
    ...
    def updateTerminalActions(self):
        self.help.portTerminalActions[self.terminalTableRow][0] = str(self.terminalLabelText.text())
        self.help.portTerminalActions[self.terminalTableRow][2] = str(self.terminalCommandText.text())
...
#           self.terminalCommandText.setStyleSheet("border: 1px solid grey;")
    ...
    def enableAutoToolsTab(self):                                       # when 'Run automated attacks' is checked this function is called
        if self.enableAutoAttacks.isChecked():
            self.AutoAttacksHelpTab.setTabEnabled(1,True)
        else:
            self.AutoAttacksHelpTab.setTabEnabled(1,False)
    ...
    def setupLayout(self):
        ...
        self.helpTabWidget = QtWidgets.QTabWidget()
        self.helpTabWidget.setTabBar(HelpTabBarWidget(width=200,height=25))
        self.helpTabWidget.setTabPosition(QtWidgets.QTabWidget.West)    # put the tab titles on the left
        
        # left tab menu items
        self.GeneralHelpTab = QtWidgets.QWidget()
        self.BruteHelpTab = QtWidgets.QWidget()
        self.ToolHelpTab = QtWidgets.QTabWidget()
        self.WordlistsHelpTab = QtWidgets.QTabWidget()
        self.AutoAttacksHelpTab = QtWidgets.QTabWidget()
        ...
        self.helpTabWidget.addTab(self.GeneralHelpTab,"General")
        self.helpTabWidget.addTab(self.BruteHelpTab,"Brute")
        self.helpTabWidget.addTab(self.ToolHelpTab,"Tools")
        self.helpTabWidget.addTab(self.WordlistsHelpTab,"Wordlists")
        self.helpTabWidget.addTab(self.AutoAttacksHelpTab,"Automated Attacks")

        self.helpTabWidget.setCurrentIndex(0)
        
        self.flayout.addWidget(self.helpTabWidget)
    ...
    def setupGeneralTab(self):
        ...
        self.vlayoutGeneral = QtWidgets.QVBoxLayout(self.GeneralHelpTab)        
    ...
    def setupBruteTab(self):
        self.vlayoutBrute = QtWidgets.QVBoxLayout(self.BruteHelpTab)
    ...
    def setupToolsTab(self):
        ...
        self.ToolHelpTab.addTab(self.ToolPathsWidget, "Tool Paths")
        self.HostActionsWidget = QtWidgets.QWidget()
        self.ToolHelpTab.addTab(self.HostActionsWidget, "Host Commands")
        self.PortActionsWidget = QtWidgets.QWidget()
        self.ToolHelpTab.addTab(self.PortActionsWidget, "Port Commands")
        self.portTerminalActionsWidget = QtWidgets.QWidget()
        self.ToolHelpTab.addTab(self.portTerminalActionsWidget, "Terminal Commands")
        self.StagedNmapWidget = QtWidgets.QWidget()
        self.ToolHelpTab.addTab(self.StagedNmapWidget, "Staged Nmap")
    ...
    def setupAutomatedAttacksTab(self):
        self.GeneralAutoHelpWidget = QtWidgets.QWidget()
        self.AutoAttacksHelpTab.addTab(self.GeneralAutoHelpWidget, "General")
        self.AutoToolsWidget = QtWidgets.QWidget()
        self.AutoAttacksHelpTab.addTab(self.AutoToolsWidget, "Tool Configuration")

        self.setupAutoAttacksGeneralTab()
        self.setupAutoAttacksToolTab()

    def setupAutoAttacksGeneralTab(self):
        self.globVerAutoSetLayout = QtWidgets.QVBoxLayout(self.GeneralAutoHelpWidget)
